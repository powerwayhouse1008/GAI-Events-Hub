      ...
      {isOrganizer && (
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border-2 border-purple-200 bg-purple-50 p-4">
          <p className="text-sm font-semibold text-purple-700">
            あなたはこのイベントの主催者です。現在の状態: {statusLabel[event.status] || event.status}。参加者、通知、資料、画像を管理できます。
          </p>
        ...
        <section>
          <div className="overflow-hidden rounded-[36px] bg-purple-100">
            {event.cover_url ? (
              ...
              <div className="grid h-[420px] place-items-center text-7xl font-black text-purple-600">AI</div>
            )}
          </div>
          ...
          {isOrganizer && (
            <div className="mt-12 space-y-8 border-t border-slate-200 pt-8">
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
                  ...
                  <div className="rounded-xl bg-orange-50 p-6">
                    ...
                    <p className="mt-2 text-3xl font-black text-orange-600">{event.capacity || "無制限"}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <button onClick={refreshParticipants} className="mb-4 rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white" type="button">
                  参加者を更新
                </button>
          ...
          {!isOrganizer && profile && (announcements.length > 0 || documents.length > 0) && (
            <div className="mt-12 space-y-8 border-t border-slate-200 pt-8">
              ...
              {documents.length > 0 && (
                <div>
                  <h2 className="mb-4 text-2xl font-black">資料</h2>
        ...
        <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
          ...
          {event.status !== "published" && !isOrganizer ? (
            ...
            <form action={registerEventAction} className="mt-8">
              ...
              <button className="btn btn-primary w-full">参加申込</button>
            </form>
          ) : !profile ? (
