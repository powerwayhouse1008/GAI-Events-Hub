...
import { footerLinkItems, normalizeFooterUrl, readStoredFooterLinks, type FooterLinkKey } from "@/lib/footer-links";
...
export function FooterFrameClient({ linkKey }: FooterFrameClientProps) {
  ...
  const safeUrl = useMemo(() => normalizeFooterUrl(readStoredFooterLinks()[item.key] || item.defaultUrl), [item.defaultUrl, item.key]);
